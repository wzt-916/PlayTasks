...
import com.jnu.student.Fragment.FinishTasksFragment;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btmNavView.setOnItemSelectedListener(item -> {
            ...
            if (item.getItemId() == R.id.navigation_statistics) {
                // 用户点击了“统计”
                Fragment finishTasksFragment = new FinishTasksFragment();
                loadTasksFragment(finishTasksFragment);
                mymenu = 2;
                return true;
            }
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        ...
        if(item.getItemId() == R.id.tasks_msg && mymenu == 2)
        {
            // 创建一个 PopupMenu
            PopupMenu popupMenu = new PopupMenu(this, findViewById(R.id.tasks_msg));
            // 在菜单中添加选项
            popupMenu.getMenu().add("已完成点数");
            popupMenu.getMenu().add("已花费点数");
            popupMenu.getMenu().add("全部删除");
            // 设置菜单项点击事件
            popupMenu.setOnMenuItemClickListener(menuItem -> {
                // 处理菜单项点击事件
                switch (menuItem.getTitle().toString()) {
                    case "全部删除":
                        ArrayList<Tasks> reward_tasks = new ArrayList<>();
                        new DataFinishTasks().SaveTasks(this,reward_tasks);
                        loadTasksFragment(new FinishTasksFragment());
                        return true;
                    case "已完成点数":
                        showDialog("已完成点数",new DataAllScore().loadScore(this));
                        return true;
                    case "已花费点数":
                        showDialog("已花费点数",new DataRewardScore().loadScore(this));
                }
                return false;
            });
            // 显示 PopupMenu
    ...
    // 创建并显示对话框的方法
    private void showDialog(String data,int score) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle(data.toString())
                .setMessage(score+"")
